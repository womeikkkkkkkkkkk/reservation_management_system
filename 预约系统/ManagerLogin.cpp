        ...
        while (ifs >> fname && ifs >> encryptedFpassward) {
            if (name == fname) {
                ...
                if (passward == decryptedPassward) {
                    //验证通过
                    logMsg = Utils::GetCurrentTime(MANAGER_LOGIN_SUCCESS);
                    ...
                    if (person == nullptr) {//内存是否分配成功
                        ...
                        logMsg = Utils::GetCurrentTime(CREATER_MANAGER_FALIED);
                        exceptionLog.LogException(logMsg);//分配失败
                    }
                    else {
                        logMsg = Utils::GetCurrentTime(CREATER_MANAGER_SUCCESS);
